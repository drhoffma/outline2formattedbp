...
\textbf{Lines 2-4} handle our imports while \textbf{Lines 7-10} parse our command line arguments. We only need a single argument here, `--image`, which is the path to our input image.
...
\begin{itemize}
    \item `--bg`: The path to the \textit{background} image.
    \item `--fg`: The path to the \textit{foreground} image.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{-{}-detector`: The path to the face detector `.xml} model.
